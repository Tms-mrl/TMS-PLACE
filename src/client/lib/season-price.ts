 ...
 *
 * Reglas (acordadas con el usuario, 2026-09-08):
 ...
 *  - Tramo por bloques: N ≤ 6 → precio por día × N · 7 ≤ N ≤ 13 → precio semana (fijo) ·
 *    N ≥ 14 → precio quincena (fijo).
 */
export function quoteForRange(seasonJson: string | null | undefined, from: string, to: string): number | null {
  ...
  const n = Math.round((end.getTime() - start.getTime()) / DAY_MS) + 1;
